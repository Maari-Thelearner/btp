...
#a=model.predict(open_image(cv2.imread(r'./img_7.jpg',cv2.IMREAD_GRAYSCALE)))
with open('./classChar.json', 'r', encoding="utf8") as f:
...
def showimages():
    ...
    cv2.imwrite('./words.jpg', final_thr)
    cv2.imwrite('./letter.jpg', src_img)
    cv2.waitKey(0)
...
def letter_seg(lines_img, x_lines, i):
    ...
    letter_index = 0
    for e in range(len(letter)):
        #print(str(letter[e][0]) + ',' + str(letter[e][1]) + ',' + str(letter[e][2]) + ',' + str(letter[e][3]) + ',' + str(e),end="")
        if letter[e][2]<mean_lttr_width*2//3 and letter[e][3]<mean_lttr_width*2//3:
            print("(.)",end="")
            continue
        if x_linescopy!=[] and (letter[e][0]<x_linescopy[0]):
            letter_index += 1
            letter_img_tmp = lines_img[i][letter[e][1]-0:letter[e][1]+letter[e][3]+5,letter[e][0]-2:letter[e][0]+letter[e][2]+2]
            letter_img = cv2.resize(letter_img_tmp, dsize =(120, 120), interpolation = cv2.INTER_AREA)
            cv2.imwrite('./segmented_img/img1/'+str(i+1)+'_'+str(word)+'_'+str(letter_index)+'.jpg', 255-letter_img)
            imgObj = open_image('./segmented_img/img1/'+str(i+1)+'_'+str(word)+'_'+str(letter_index)+'.jpg')
            prediction = model.predict(imgObj)
            m = max(prediction[0])
            prediction=[i for i, j in enumerate(prediction[0]) if j == m]
            print(classChar[("00"+str(prediction[0]))[-3:]],end="")
        elif x_linescopy!=[]:
            x_linescopy.pop(0)
            word += 1
            letter_index = 1
...
print("\n........Program Initiated.......\n")
src_img= cv2.imread('./img_7.jpg', 1)
copy = src_img.copy()
...
cv2.imwrite('./processed.jpg', 255-final_thr)
r
...
for y in range(len(count_x)):
    if y >= 100 and y <= len(count_x)-101:
        arr1 = count_x[y-100:y+100]
    elif y < 100:
        arr1 = count_x[0:y+100]
    else:
        arr1 = count_x[y-100:len(count_x)-1]
