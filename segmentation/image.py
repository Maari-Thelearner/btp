...
def open_image(src):
    img1 = Image.open(src)
    img1.save(r'./cache.png', 'PNG')
    src=cv2.imread(r'./cache.png',cv2.IMREAD_GRAYSCALE)
    ...
    cv2.imwrite("./cache/processed.png", centered)
    img = convert_image_to_1d_array(centered)
    img=img.reshape(1,90,90,1)
    return img
